...
int main(int argc, char** argv) {
    ...
    *reinterpret_cast<int*>(&remote_ipc_handle) = remote_fd;
    ...
    //           << " (from rank " << remote_rank_id << ")" << std::endl;
    // to hold both local and remote sync pointers
    int32_t** usm_sync_buffers = (int32_t**)sycl::malloc_shared<int32_t*>(num_elements * world_size, compute_queue);
    compute_queue.memset(usm_sync_buffers, 0, num_elements * world_size * sizeof(int32_t*)).wait();

    for (int i = 0; i < world_size; i++) {
      for (int j = 0; j < num_elements; j++) {
        if (i == rank) {
           usm_sync_buffers[i*world_size + j] = local_ptr + j;
        } else { // world_size must be 2 here, so the other rank is remote rank
           usm_sync_buffers[i*world_size + j] = remote_ptr + j;
        }
        std::cout << "node: " << rank << ", address[" << i << "," << j <<"], is " << usm_sync_buffers[i*world_size + j] << std::endl;
      }
    }
      ...
      compute_queue.submit([&](sycl::handler &h) {
        ...
        h.parallel_for(sycl::nd_range<1>(sycl::range<1>(num_elements), sycl::range<1>(num_elements)), [=](sycl::nd_item<1> item) {
            ...
            if (tid < num_elements) {
                ...
                int32_t *sync_buffer_dst = usm_sync_buffers[tid*world_size + rank];
                //out << "node: " << rank << " sync_buffer_dst tid: " << tid << ", rank: " << rank << ", address: " << sync_buffer_dst << sycl::endl;
                ...
                *sync_buffer_dst = 1;
                ...
                int32_t *wait_ptr = usm_sync_buffers[rank*world_size + tid];
                //out << "node: " << rank << " wait_ptr tid: " << tid << ", rank: " << rank << ", address: " << wait_ptr << sycl::endl;
    ...
    sycl::free(usm_sync_buffers, compute_queue);
    close(accept_sock);
    close(client_sock);
    close(server_sock);
    unlink(server_name);

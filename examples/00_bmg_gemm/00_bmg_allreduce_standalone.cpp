...
struct Options {
  ...
  std::ostream& print_usage(std::ostream& out) const {
    out << "Standalone BMG Allreduce Example\\n\\n"
...
class StandaloneAllreduceKernelName;
...
struct Runner {
  ...
  void launch_allreduce(
    ...
    if ((n_elems % BF16_VEC) != 0) {
      throw std::runtime_error("n_elems must be divisible by NUM_PER_TH*4 for vector allreduce.");
    }

    int wg_size = static_cast<int>(
        q_->get_device().get_info<sycl::info::device::max_work_group_size>());
    wg_size = std::max(64, std::min(wg_size, 256));

    int64_t i64_elems = n_elems / BF16_PER_I64;
    int64_t vec_elems = i64_elems / NUM_PER_TH;
    int64_t groups = (vec_elems + wg_size - 1) / wg_size;

    q_->submit([&](sycl::handler& h) {
      h.parallel_for<StandaloneAllreduceKernelName<NUM_PER_TH>>(
          sycl::nd_range<1>(
              sycl::range<1>(static_cast<size_t>(groups * wg_size)),
              sycl::range<1>(static_cast<size_t>(wg_size))),
          [=](sycl::nd_item<1> item) {
            allreduce_device<NUM_PER_TH>(item, ipc_data_ptrs,
  ...
  bool run(Options const& options, sycl::device const& device, int rank, int world_size) {
    ...
    // Fill each rank's own slot with (rank+1) so allreduce sum = 1+2+...+world_size
    ...
    q_->submit([&](sycl::handler& h) {
      h.parallel_for(sycl::range<1>(static_cast<size_t>(n_elems)), [=](sycl::id<1> i) {
        local_slot[i] = static_cast<Element>(static_cast<float>(rank + 1));
      });
    });
    q_->wait();
    ...
    for (int i = 0; i < 5; ++i) {
      launch_allreduce<NUM_PER_TH>(dev_slot_ptrs, out, world_size, n_elems);
    }
    q_->wait();
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      ...
      std::cout << "Allreduce standalone: m=" << m
                << " n=" << n
